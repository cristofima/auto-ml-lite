...
class AutoAnomaly:
    ...
    def fit(self, df: pd.DataFrame, algorithm: str = "auto") -> "AutoAnomaly":
        ...
        """
        print(
            f"Starting anomaly detection training (Contamination: {self.contamination}, Algo: {algorithm})..."
        )
        ...
        if algorithm == "lof":
            ...
            self.model = LocalOutlierFactor(
                contamination=self.contamination, novelty=True, n_neighbors=20
            )
        elif algorithm == "svm":
        ...
        self.dataset_info = {"rows": len(df), "features": len(self.feature_columns)}
        ...
        print(
            f"Analysis complete. Found {anomaly_count} anomalies ({self.metrics['anomaly_percentage']:.2f}%)."
        )
        return self
