...
def generate_eda_report(
    ...
    try:
        ...
        print(f"Error generating EDA report: {e!s}")
        ...
        for col in self.features.columns:
            ...
            if series.dtype == "object" and series.nunique() / len(series) > 0.5:
                self.excluded_columns.append(
                    (col, f"High cardinality ({series.nunique()} unique values)")
                )
    ...
    def _generate_unsupervised_analysis(self) -> str:
        ...
        if len(numeric_df.columns) >= 2:
            try:
                ...
                for x, y in zip(norm_x, norm_y, strict=True):
    ...
    def _generate_warnings(self) -> str:
        ...
        if self.warnings:
            for w in self.warnings:
                html += f'<div class="warning-box">{w}</div>'
        html += "</div>"
        return html
